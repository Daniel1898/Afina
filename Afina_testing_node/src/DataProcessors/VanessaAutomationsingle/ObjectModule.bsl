...
#Область ОтчетДляСистемыТестирования

//Николаев Д.Н Начало 09.05.2023
//Пример выходного файла
//{
//  "date":"09.03.2023",
//	"id":"decfb4f0-35e2-4963-8cae-372c7c44320d",
//	"result": 23,
//  "tests":[
//			    {
//					"id":"decfb4f0-35e2-4963-8cae-372c7c44323d",
//					"result":"success"
//					//todo: массив шагов
//				}
//			]
//}
Процедура СформироватьОтчетДляСистемыТестирования(СтруктураОФ) Экспорт 
	...
	ЗаписатьJSON(ЗаписьJson, ОбойтиДеревоДляОтчетаСистемыТестирования(СтруктураОФ.МассивРезультатПрохожденияТестовСценария, ДеревоТестов.Строки[0].Строки));
...
Функция ОбойтиДеревоДляОтчетаСистемыТестирования(РезультатыТестирования, ИнформацияОТестах)

	Результат = Новый Структура;
	Результат.Вставить("result",0);
	Результат.Вставить("scenaries",Новый Массив);
	
	ТекущийТест = 0;
	Для каждого Тест Из РезультатыТестирования Цикл
		
		ИдентификаторТеста = ИнформацияОТестах[ТекущийТест].Имя;
		
		Сценарий = Новый Структура;
		Сценарий.Вставить("status", Тест.Статус);
		Сценарий.Вставить("id", ИдентификаторТеста); 
		
		Если Тест.Статус = "Success" Тогда
		
			Результат.result = Результат.result + 1;
		
		КонецЕсли; 
		
		Шаги = Новый Массив;
		
		Для каждого Шаг Из Тест.РезультатПрохожденияТестовШагов Цикл
		
			ТекущийШаг = Новый Структура;
			ТекущийШаг.Вставить("status", Шаг.Статус);
			ТекущийШаг.Вставить("description", Шаг.Имя);
			Шаги.Добавить(ТекущийШаг);
			
		КонецЦикла;
		
		Сценарий.Вставить("steps", Шаги);
		Результат.scenaries.Добавить(Сценарий);
		
		ТекущийТест = ТекущийТест + 1;
	КонецЦикла;
