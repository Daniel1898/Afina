...
Процедура АктуализироватьЗначение(Команда)
	...
	Если НомерСтрокиДерева > 0 Тогда
		ПоискНачалаСтроки = "текущее значение равно:";
		ТехническиеДанные = "Технические данные:";
		ВозможнаяСтрокаШаблона = "Возможная строка шаблона <";
		АОжидалось = ", а ожидалось:";  
		НомерСтрокиФичи = "НомерСтрокиФичи:";
		ПолныйПутьКФиче = "ПолныйПутьКФиче:";
		
		//Поиск полученного значения      
				
		НачалоСтроки = Найти(ТекстОшибки, ПоискНачалаСтроки); 
		НачалоСтроки = НачалоСтроки + СтрДлина(ПоискНачалаСтроки);
		КонецСтроки = Найти(ТекстОшибки, АОжидалось); 
		СколькоСимволовИскать = КонецСтроки - НачалоСтроки; 
	
		ПолученноеЗначение = СокрЛП(Сред(ТекстОшибки, НачалоСтроки, СколькоСимволовИскать));
		
		//Поиск эталонного значения
		
		//В тексте ошибки может быть текст про Текущую строку шаблона
		Если Найти(ТекстОшибки, ВозможнаяСтрокаШаблона) > 0 Тогда
			КонецСтрокиЭталон = ВозможнаяСтрокаШаблона
		Иначе
			КонецСтрокиЭталон = ТехническиеДанные
		КонецЕсли;
		
		НачалоСтроки = Найти(ТекстОшибки, АОжидалось); 
		НачалоСтроки = НачалоСтроки + СтрДлина(АОжидалось);
		КонецСтроки = Найти(ТекстОшибки, КонецСтрокиЭталон);
		СколькоСимволовИскать = КонецСтроки - НачалоСтроки;   
		
		ЭталонноеЗначение = СокрЛП(Сред(ТекстОшибки, НачалоСтроки, СколькоСимволовИскать));
		
		//Поиск строки фичи
		НачалоСтроки = Найти(ТекстОшибки, НомерСтрокиФичи); 
		НачалоСтроки = НачалоСтроки + СтрДлина(НомерСтрокиФичи);
		КонецСтроки = Найти(ТекстОшибки, ПолныйПутьКФиче);
		СколькоСимволовИскать = КонецСтроки - НачалоСтроки; 
		
		НомерСтрокиФичи = Число(СокрЛП(Сред(ТекстОшибки, НачалоСтроки, СколькоСимволовИскать)));		
		
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("НомерСтрокиФичи", НомерСтрокиФичи);
		СтруктураПараметров.Вставить("ПолученноеЗначение", ТаблицаЭталон);
		СтруктураПараметров.Вставить("ЭталонноеЗначение", ЭталонноеЗначение);
		
		Оповестить("АктуализироватьЗначениеТекст", СтруктураПараметров);
	КонецЕсли;	 
...
&НаКлиенте
Процедура АктуализироватьТаблицу(Команда)  
	...
	Если НомерСтрокиДерева > 0 Тогда
		...
		СтруктураПараметров.Вставить("НомерСтрокиФичи", НомерСтрокиФичи);
		СтруктураПараметров.Вставить("ПолученноеЗначение", ТаблицаЭталон);
		СтруктураПараметров.Вставить("ЭталонноеЗначение", ТаблицаТекущееЗначение);
		
		Оповестить("АктуализироватьЗначениеТаблица", СтруктураПараметров);
	КонецЕсли;	 
